...
// Mock sub-modules directly (not the barrel) so ESM live bindings in the source
// file resolve to the mock functions rather than the original implementations.
vi.mock("../../config/sessions/store.js", async (importOriginal) => {
  ...
  return { ...actual, updateSessionStore: vi.fn() };
});

vi.mock("../../config/sessions/artifacts.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../../config/sessions/artifacts.js")>();
  return {
    ...actual,
