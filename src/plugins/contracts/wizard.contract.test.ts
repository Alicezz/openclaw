...
describe("provider wizard contract", () => {
  ...
  it("exposes every registered provider setup choice through the shared wizard layer", () => {
    ...
    const expectedValues = resolveExpectedWizardChoiceValues(uniqueProviderContractProviders);
    expect(optionValues).toEqual([...new Set(optionValues)]);
    expect(optionValues).not.toContain("copilot-proxy");
  ...
  it("exposes every registered model-picker entry through the shared wizard layer", () => {
    ...
    expect(entryValues).toEqual([...new Set(entryValues)]);
