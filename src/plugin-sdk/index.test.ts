...
describe("plugin-sdk exports", () => {
  ...
  it("emits importable bundled subpath entries", { timeout: 240_000 }, async () => {
    ...
    const pluginSdkDistDir = path.join(repoDistDir, "plugin-sdk");

    try {
      // CI/unit runs do not always build dist artifacts first.
      if (!(await pathExists(pluginSdkDistDir))) {
        return;
      }

      for (const entry of pluginSdkEntrypoints) {
        const module = await import(pathToFileURL(path.join(pluginSdkDistDir, `${entry}.js`)).href);
