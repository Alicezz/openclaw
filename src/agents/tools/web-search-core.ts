...
async function runGrokSearch(params: {
  ...
  return withTrustedWebSearchEndpoint(
    ...
    // Convert AbortError (from fetch timeout) to a plain Error so callers
    // that do not expect DOMException / AbortError propagate a clear message.
    if (isAbortError(err)) {
      throw new Error(`Grok web search timed out after ${params.timeoutSeconds}s`, { cause: err });
    }
    throw err;
  });
}
